...
This section details the present commands, command flags in the Jury, and how they're used.
...
Used to specify the data directory used for storing Jury client data. Default: `./test-chain`.
...
<Tabs>
  ...
  <TabItem value="example" label="Example">
    ...
    server --access-control-allow-origins "https://jury-docs.doge.technology"
...
Available options: `TRACE, DEBUG, INFO, WARN, ERROR, OFF`.        
Default: `INFO`.
...
Sets the premined accounts and balances in the format `address:amount` in WEI.     
...
Sets the directory for the Jury genesis data. Default: `./genesis.json`.
...
Sets the name for the chain. Default: `jury`.
...
| secrets generate         | Generates a secrets manager configuration file which can be parsed by the Jury    |
...
Sets the directory for the Jury data if the local FS is used.
...
Specifies the name of the node for on-service record keeping. Default: `jury-node`
