...
Used to specify the data directory used for storing dogechain client data. Default: `./test-chain`.
...
<h4><i>max-account-demotions</i></h4>
<Tabs>
  <TabItem value="syntax" label="Syntax" default>

    server [--max-account-demotions MAX_DEMOTIONS]

  </TabItem>
  <TabItem value="example" label="Example">

    server --max-account-demotions 5

  </TabItem>
</Tabs>

Sets the flag for maximum account demontion counter limit in the pool. Default: `10`.
...
Account in the pool not promoted for a long time would be pruned. Default: `1800`.
