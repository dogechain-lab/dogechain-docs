...
To accomplish this task, the dogechain leverages the battle-tested **libp2p** framework.
...
* **Modularity** - libp2p is modular by nature, just like the dogechain. This gives greater flexibility, especially when parts of the dogechain need to be swappable

## GRPC

On top of **libp2p**, the dogechain uses the **GRPC** protocol. <br />
Technically, the dogechain uses several GRPC protocols, which will be covered later on.

The GRPC layer helps abstract all the request/reply protocols and simplifies the streaming protocols needed for the dogechain to function.
...
Earlier, we mentioned that the dogechain leverages several GRPC protocols.<br />
...
The dogechain also implements several service methods that are used by other nodes on the network. <br />
